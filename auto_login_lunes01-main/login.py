...
def _set_tg_context(token: str, chat: str, prefix: str):
    global _CUR_TG_TOKEN, _CUR_TG_CHAT, _CUR_TG_PREFIX
    _CUR_TG_TOKEN = (token or "").strip()
    _CUR_TG_CHAT = (chat or "").strip()
    _CUR_TG_PREFIX = (prefix or "").strip()
...
def tg_send_photo(
    ...
    try:
        ...
        # ✅ 仅加功能：如果发图失败，至少发一条文字告诉你
        print(f"⚠️ TG 发图失败：{e}")
        tg_send(f"⚠️ TG 发图失败：{e}\n{caption}", token, chat_id)


def _send_any_screenshot_to_tg(sb: SB, server_id: Optional[str], stage: str):
    """
    ✅ 新增：在任何“退出前流程”失败点，也截图并发 TG（这样你就不会再遇到“没截图”）
    """
    ts = int(time.time())
    sid = server_id or "unknown"
    name = f"before_logout_{stage}_{sid}_{ts}.png"
    path = f"{SCREENSHOT_DIR}/{name}"
    try:
        sb.save_screenshot(path)
        print(f"📸 {path}")
    except Exception as e:
        print(f"⚠️ 截图失败（{stage}）：{e}")
        return

    try:
        url_now = (sb.get_current_url() or "").strip()
    except Exception:
        url_now = ""
    ...
    if _CUR_TG_TOKEN and _CUR_TG_CHAT:
        tg_send_photo(path, caption, _CUR_TG_TOKEN, _CUR_TG_CHAT)
...
def _post_login_visit_then_logout(sb: SB) -> Tuple[Optional[str], bool]:
    ...
    if not entered_ok:
        # ✅ 新增：在“退出前流程”提前失败时也截图+发TG
        _send_any_screenshot_to_tg(sb, server_id, "enter_server_failed")
        return server_id, False
    ...
    try:
        ...
        # ✅ 新增：回首页失败时也截图+发TG（这就是你这次最可能发生的情况之一）
        _send_any_screenshot_to_tg(sb, server_id, "back_home_failed")
        return server_id, False
    ...
    try:
        ...
        # ✅ 新增：不管退出成功/失败，退出前都截图并发 TG
        _send_any_screenshot_to_tg(sb, server_id, "before_click_logout")

        sb.click(LOGOUT_SEL)
    except Exception:
        # ✅ 新增：点退出失败时，也截图并发 TG
        _send_any_screenshot_to_tg(sb, server_id, "logout_click_failed")
    ...
    # ✅ 新增：退出验证失败也截图+发TG
    _send_any_screenshot_to_tg(sb, server_id, "logout_verify_failed")
    return server_id, False
...
def main():
    ...
    try:
        for i, acc in enumerate(accounts, start=1):
            ...
            # ✅ 新增：给“截图发 TG”设置当前账号的 TG 上下文
            _set_tg_context(tg_token, tg_chat, f"账号：{safe_email}")
